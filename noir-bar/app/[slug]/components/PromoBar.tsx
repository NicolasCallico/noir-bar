...
export function PromoBar({ promotions }: Props) {
  ...
  return (
    <div style={{ position: "sticky", top: 0, zIndex: 31, backgroundColor: "#0D0D0D", borderBottom: "1px solid rgba(200,169,107,0.1)" }}>
            ...
            <div className="flex justify-center gap-1 mt-1.5">
              {promotions.map((_, i) => (
                <button
                  ...
                  className={`rounded-full transition-all duration-300 ${i === current ? "bg-gold w-3 h-1.5" : "bg-gold/30 w-1.5 h-1.5"}`}
                />
              ))}
            </div>
          )}
